...
if ( !-e "./$infile" ) {
    ...
    print
"a        Parameter Options : [Boundary = $BoundaryCondition], [SON = $SON], [Double Power Rail = $DoublePowerRail], [MPL = $MPL_Parameter], [Maximum Metal Layer = $MM_Parameter], [Localization = $Local_Parameter]\n";
    ...
    if ( $infileStatus eq "net" ) {
        if ( $line =~ /^i   net(\S+)\s*(\d+)PinNet/ ) {
            ...
            for my $pinIndex_inNet ( 0 .. $#net ) {
                if ( exists( $h_outpinId_idx{ $net[$pinIndex_inNet] } )
                    && ( $pins[ $h_outpinId_idx{ $net[$pinIndex_inNet] } ][7] eq "VDD" || $pins[ $h_outpinId_idx{ $net[$pinIndex_inNet] } ][7] eq "VSS" ) )
                {
                    $powerinNet = 1;
...
print $out "(define-fun max ((x (_ BitVec "
  . ( length( sprintf( "%b", $newNumTrackV ) ) + 4 )
  . ")) (y (_ BitVec "
  . ( length( sprintf( "%b", $newNumTrackV ) ) + 4 )
  . "))) (_ BitVec "
  . ( length( sprintf( "%b", $newNumTrackV ) ) + 4 ) . ")\n";
                                        ...
                                        if ( !exists( $h_tmp{ $tmp_vname1 . $tmp_vname2 } ) ) {
                                            if ( !exists( $h_assign{"M_$tmp_vname1\_$tmp_vname2"} ) ) {
                                                ...
                                                $str .=
                                                  ...
                                                  . ( $col > 0 ? ( $col - $metalOneStepSize ) : $col )
                                                  . " $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= M_$tmp_vname1\_$tmp_vname2 false)))\n";
    ...
    for my $vEdgeIndex ( 0 .. $#virtualEdges ) {
        ...
        if ( $minWidth >= 2 && $toCol > ( $minWidth - 2 ) * $metalOneStepSize ) {
            if ( !exists( $h_assign{"M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"} ) ) {
                ...
                $str .=
                  "(assert (ite (bvsle COST_SIZE (_ bv" . ( $toCol - ( 2 * $metalOneStepSize ) ) . " $len)) (= M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)))\n";
                ...
                if ( $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3] ) {
                    if ( !exists( $h_assign{"N$nets[$netIndex][1]_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"} ) ) {
                        ...
                        $str .=
                          ...
                          . ( $toCol - ( 2 * $metalOneStepSize ) )
                          . " $len)) (= N$nets[$netIndex][1]_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)))\n";
                        ...
                        if ( $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex] ) {
                            if ( !exists( $h_assign{"N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"} ) ) {
                                ...
                                $str .=
                                    "(assert (ite (bvsle COST_SIZE (_ bv"
                                  . ( $toCol - ( 2 * $metalOneStepSize ) )
                                  . " $len)) (= N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)))\n";
                    ...
                    if ( $cnt_var_T > 0 || $cnt_var_F > 0 ) {
                        ...
                        if (
                            (
                                ...
                                ? ( ( $startIdx + $endIdx_s - 1 - $tmp_pidx_s ) ) * $metalOneStepSize
                                : ( ( $startIdx + $endIdx_s - 1 - $tmp_pidx_s + 1 ) * $metalOneStepSize )
                            ) <= $col
                          )
                        {
                            if ( $isValid == 0 ) {
                        ...
                        if (
                            (
                                ...
                                ? ( $startIdx + $endIdx_t - 1 - $tmp_pidx_t ) * $metalOneStepSize
                                : ( $startIdx + $endIdx_t - 1 - $tmp_pidx_t + 1 ) * $metalOneStepSize
                            ) <= $col
                          )
                        {
                            if ( $isValid == 0 ) {
                            ...
                            if ( $tmp_pidx >= $startIdx && $tmp_pidx <= $endIdx - 1 ) {
                                if ( $tmp_pidx % 4 == 0 ) {
                                    if ( $j == 0 ) {
                                        if (
                                            (
                                                  ( ( $startIdx + $endIdx - 1 - $tmp_pidx ) % 2 == 0 )
                                                ? ( ( $startIdx + $endIdx - 1 - $tmp_pidx ) * $metalOneStepSize )
                                                : ( ( $startIdx + $endIdx - 1 - $tmp_pidx + 1 ) * $metalOneStepSize )
                                            ) <= $col
                                          )
                                        {
                                            if ( $isValid == 0 ) {
            ...
            for my $commodityIndex ( 0 .. $nets[$netIndex][4] - 1 ) {
                ...
                if ( $nets[$netIndex][5][$commodityIndex] ne $keySON ) {
                    ...
                    for ( my $col = 0 ; $col < $newNumTrackV ; $col += $metalOneStepSize ) {
                        ...
                        $str .=
                            "(assert (ite (and (= ff$inst_pin_s false) (= ff$inst_pin_t false)) (ite (bvsge (bvadd x$inst_pin_s (_ bv"
                          . $pidx_s * $metalOneStepSize
                          . " $len)) (bvadd x$inst_pin_t (_ bv"
                          . $pidx_t * $metalOneStepSize
                          . " $len)))\n";
                        cnt( "l", 0 );
                        cnt( "l", 0 );
                        cnt( "l", 0 );
                        cnt( "l", 0 );
                        $str .=
                            "             (and (ite (bvslt x$inst_pin_s (_ bv"
                          . ( ( $col - ( $tolerance + $pidx_s ) * $metalOneStepSize >= 0 ) ? ( $col - ( $tolerance + $pidx_s ) * $metalOneStepSize ) : (0) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        ...
                        $str .= ") (= true true))\n";
                        $str .=
                          "                  (ite (bvsgt x$inst_pin_t (_ bv"
                          . ( ( $col + ( $tolerance - $pidx_t ) * $metalOneStepSize >= $newNumTrackV - 1 )
                            ? ( $newNumTrackV - 1 )
                            : ( ( $col + ( $tolerance - $pidx_t ) * $metalOneStepSize >= 0 ) ? ( $col + ( $tolerance - $pidx_t ) * $metalOneStepSize ) : (0) ) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        %h_edge = ();
                        for my $row ( 0 .. $numTrackH - 3 ) {
                        ...
                        $str .= ") (= true true)))\n";
                        $str .=
                            "             (and (ite (bvslt x$inst_pin_t (_ bv"
                          . ( ( $col - ( $tolerance + $pidx_t ) * $metalOneStepSize >= 0 ) ? ( $col - ( $tolerance + $pidx_t ) * $metalOneStepSize ) : (0) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        %h_edge = ();
                        ...
                        $str .= ") (= true true))\n";
                        $str .=
                          "                  (ite (bvsgt x$inst_pin_s (_ bv"
                          . ( ( $col + ( $tolerance - $pidx_s ) * $metalOneStepSize >= $newNumTrackV - 1 )
                            ? ( $newNumTrackV - 1 )
                            : ( ( $col + ( $tolerance - $pidx_s ) * $metalOneStepSize >= 0 ) ? ( $col + ( $tolerance - $pidx_s ) * $metalOneStepSize ) : (0) ) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        ...
                        $str .= ") (= true true))))\n";
                        $str .=
                            "	(ite (and (= ff$inst_pin_s false) (= ff$inst_pin_t true)) (ite (bvsge (bvadd x$inst_pin_s (_ bv"
                          . $pidx_s * $metalOneStepSize
                          . " $len)) (bvadd x$inst_pin_t (_ bv"
                          . ( $w_t - $pidx_t ) * $metalOneStepSize
                          . " $len)))\n";
                        cnt( "l", 0 );
                        cnt( "l", 0 );
                        cnt( "l", 0 );
                        ...
                        $str .=
                          "                  (ite (bvsgt x$inst_pin_t (_ bv"
                          . ( ( $col + ( $tolerance - $w_t + $pidx_t ) * $metalOneStepSize >= $newNumTrackV - 1 )
                            ? ( $newNumTrackV - 1 )
                            : ( $col + ( $tolerance - $w_t + $pidx_t ) * $metalOneStepSize >= 0 ? ( $col + ( $tolerance - $w_t + $pidx_t ) * $metalOneStepSize ) : (0) ) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        %h_edge = ();
                        for my $row ( 0 .. $numTrackH - 3 ) {
                        ...
                        $str .= ") (= true true))))\n";
                        $str .=
                            "	(ite (and (= ff$inst_pin_s true) (= ff$inst_pin_t false)) (ite (bvsge (bvadd x$inst_pin_s (_ bv"
                          . ( $w_s - $pidx_s ) * $metalOneStepSize
                          . " $len)) (bvadd x$inst_pin_t (_ bv"
                          . $pidx_t * $metalOneStepSize
                          . " $len)))\n";
                        cnt( "l", 0 );
                        ...
                        $str .=
                            "             (and (ite (bvslt x$inst_pin_s (_ bv"
                          . ( ( $col - ( $tolerance + $w_s - $pidx_s ) * $metalOneStepSize >= 0 ) ? ( $col - ( $tolerance + $w_s - $pidx_s ) * $metalOneStepSize ) : (0) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        %h_edge = ();
                        ...
                        $str .=
                          "                  (ite (bvsgt x$inst_pin_s (_ bv"
                          . ( $col + ( $tolerance - $w_s + $pidx_s ) * $metalOneStepSize >= $newNumTrackV - 1
                            ? ( $newNumTrackV - 1 )
                            : ( $col + ( $tolerance - $w_s + $pidx_s ) * $metalOneStepSize >= 0 ? ( $col + ( $tolerance - $w_s + $pidx_s ) * $metalOneStepSize ) : (0) ) )
                          . " $len)) (and";
                        cnt( "l", 0 );
                        ...
                        $str .= ") (= true true))))\n";
                        $str .=
                            "	(ite (bvsge (bvadd x$inst_pin_s (_ bv"
                          . ( $w_s - $pidx_s ) * $metalOneStepSize
                          . " $len)) (bvadd x$inst_pin_t (_ bv"
                          . ( $w_t - $pidx_t ) * $metalOneStepSize
                          . " $len)))\n";
...
if ( $MM_Parameter >= 4 ) {
    ...
    if ( $Partition_Parameter == 2 && $num_netopt > 0 ) {
        ...
        # YW: following loop only concerns M3~M4
        my $stepSize = $metalThreeStepSize;
